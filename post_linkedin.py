...
def post(token, author_urn, text, images=None):
    assets = []

    if images:
        print(f"📤 Uploading {len(images)} image(s)...")
        for img in images[:2]:
            a = upload_image(token, author_urn, img)
            if a:
                assets.append(a)
    ...
    if assets:
        payload["specificContent"]["com.linkedin.ugc.ShareContent"]["media"] = [
            {
                "status": "READY",
                "description": {"text": "AI News"},
                "media": a,
                "title": {"text": "Top AI News"},
            }
            for a in assets
        ]
    ...
    return requests.post(
        f"{API}/ugcPosts", headers=headers(token), json=payload, timeout=30
    )
...
if __name__ == "__main__":
    ...
    print(f"📝 Post length: {len(text)}")

    imgs = []
    if Path("images.json").exists():
        with open("images.json") as f:
            imgs = [
                i for i in json.load(f).get("images", [])
                if Path(i).exists()
            ]

    print(f"🖼 Images: {imgs}")

    resp = post(token, author_urn, text, imgs)
